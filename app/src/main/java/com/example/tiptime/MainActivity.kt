...
fun TipTimeLayout() {
    ...
    Column(
        ...
        RoundTheTipRow( // parametre roundUp defini sur roundup et rappel lambda qui màj la valeur de roundUp
            roundUp = roundUp,
...
fun RoundTheTipRow(
    roundUp: Boolean,
    onRoundUpChanged: (Boolean) -> Unit,// hisse l'etat du btn bascule
    modifier: Modifier = Modifier
) {
    Row(
        modifier = modifier
        ...
        Switch(
            ...
            checked = roundUp, // check si bouton activé
            onCheckedChange = onRoundUpChanged, // rappel à appeler en cas de clic sur le btn
        )
    }
}
...
private fun calculateTip(amount: Double, tipPercent: Double = 15.0, roundUp: Boolean): String {
    //val tip = tipPercent / 100 * amount // erreur avec cette ligne apres roundup
    var tip = tipPercent / 100 * amount
    if(roundUp){
        tip = kotlin.math.ceil(tip) // arrondit si btn basule d'arrondi activé
    }
